...
if (wrap) {
	...
	function createCard(g: SerializedGame): HTMLElement {
		...
		const safeImage = g.image ? escapeHtml(g.image) : '';
		const safeSlug = escapeHtml(slug);
		const safeType = escapeHtml(g.type);
		a.innerHTML = `
			<div class="card-cover">
				${g.image
					? `<img src="/${safeImage}" alt="" loading="lazy" decoding="async" width="400" height="225">`
					: '<div class="card-cover-empty" aria-hidden="true"></div>'}
				<span class="card-type card-type--${safeType}" title="${safeType}">
				...
				<button class="card-fav" type="button" aria-label="Favorite ${safeName}" data-fav-toggle="${safeSlug}">
			...
			<div class="card-meta">
				...
				${g.tags?.length ? `<div class="card-sub">${escapeHtml(g.tags.slice(0, 2).join(' · '))}</div>` : ''}
			</div>
		`;
		return a;
	}

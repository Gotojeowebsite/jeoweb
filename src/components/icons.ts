/**
 * Lucide-derived inline SVG paths. Authored as a flat record so we can ship
 * only the paths we use and never pull in lucide as a runtime dep.
 */
export type IconName =
	| 'zap' | 'gamepad' | 'flame' | 'heart' | 'heart-filled'
	| 'bookmark' | 'library' | 'history' | 'sparkles' | 'clipboard'
	| 'dice' | 'moon' | 'sun' | 'settings' | 'palette'
	| 'shield' | 'package' | 'bar-chart' | 'info' | 'maximize'
	| 'bug' | 'external' | 'link' | 'save' | 'trophy'
	| 'search' | 'x' | 'chevron-left' | 'chevron-right'
...
export const ICON_PATHS: Record<IconName, string> = {
	...
	'cloud':         '<path d="M17.5 19a4.5 4.5 0 1 0-1.41-8.775 5.5 5.5 0 0 0-10.7 1.022A4.5 4.5 0 0 0 6.5 19h11Z"/>',

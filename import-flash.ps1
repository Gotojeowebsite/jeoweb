...
#      Windows PowerShell: .\import-flash.ps1
#      Codespaces/Linux:   pwsh ./import-flash.ps1
...
if ($imported -gt 0) {
    ...
    function Find-Image($folderPath, $folderName) {
        ...
        $normalizedFolderPath = [System.IO.Path]::GetFullPath($folderPath)
        ...
        foreach ($pn in $priorityNames) {
            ...
            if ($match) {
                $rel = [System.IO.Path]::GetRelativePath($normalizedFolderPath, $match.FullName).Replace('\\','/')
        ...
        $rel = [System.IO.Path]::GetRelativePath($normalizedFolderPath, $first.FullName).Replace('\\','/')
        return "Assets/$folderName/$rel"
    }
